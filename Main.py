...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"Justin{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/Justin.sfc"
    ...
    # BT Gray Doors Moved
    romWriter.writeBytes(0x7e9d9, b"\x26")
    romWriter.writeBytes(0x7e9eb, b"\x26")

    # Plasma patch
    romWriter.writeBytes(0x1a4d0, b"\x00\x80")
